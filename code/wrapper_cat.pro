pro wrapper_cat
    ...
    ind2=where(cat.flags ne 3 or cat.mag_auto lt 21 or cat.mag_auto gt 25)
    cat2=cat[ind2]

    maxima_ind=(long(cat.x_image)+long(cat.y_image)*sizex)
    count_area=dblarr(n_elements(maxima_ind))+1
    purge_area, maxima_ind, image, 27, count_area=count_area

    external_x2=long(cat2.x_image)
    external_y2=long(cat2.y_image)
    purge_match, image, maxima_ind, count_area, external_x2, external_y2, 10

    res_str=der_residual(image, weight, maxima_ind, count_area) ; derive residual image using detection image
    tempres_name='./example/temp_res.fits'
    arrange_stamps1, res_str, tempres_name, hdr_img, hdr_wt
    

    filename=tempres_name
    filepsf='./example/eg_image_psfcat.psf'
    cat_out='./example/eg_catdeb.fits'
